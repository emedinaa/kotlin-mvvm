...
import com.emedinaa.kotlinmvvm.fake.FakeErrorMuseumATRepository
...
class MuseumErrorActivityTest {
    ...
    @Test
    fun displayLayoutErrorWithFakeRepository(){
        val activityScenario = ActivityScenario.launch(MuseumActivity::class.java)
        onView(withId(R.id.layoutEmpty)).check(matches(withEffectiveVisibility(Visibility.GONE)))
        onView(withId(R.id.recyclerView)).check(matches(hasChildCount(0)))
        onView(withId(R.id.layoutError)).check(matches(isDisplayed()))
        onView(withId(R.id.textViewError)).check(matches(withText("Error ${Exception("Ocurrió un error")}")))
    ...
    object MVVMModule {
        ...
        fun providerRepository(): MuseumDataSource {
            return FakeErrorMuseumATRepository()
        }
    }
}